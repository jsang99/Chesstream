...
QueueHandle_t xQueueFENStringBuffer = NULL; ///< Queue to send the FEN String to the cloud
QueueHandle_t xQueueChessInputBuffer = NULL; ///< Queue to send the FEN String to the cloud // TODO
...
static void MQTT_HandleFENStringMessages(void);
static void MQTT_HandleChessInputMessages(void);// TODO
...
void SubscribeHandlerBestMoveTopic(MessageData *msgData) // TODO
{
	if (startup != 1 && globalTrackPlayer == 'b'){
		
 		LogMessage(LOG_DEBUG_LVL, "\r\nChess Best Move topic received!\r\n");
 		LogMessage(LOG_DEBUG_LVL, "\r\n %.*s ", msgData->topicName->lenstring.len, msgData->topicName->lenstring.data);
 		LogMessage(LOG_DEBUG_LVL, "%.*s", msgData->message->payloadlen, (char *)msgData->message->payload);
		
		char* tempstr = msgData->message->payload;
		
		 
		uint8_t pieceToMoveCol = tempstr[0] - MAGIC_CONVERSION + 1;
		uint8_t pieceToMoveRow = tempstr[1] - 48;
		uint8_t squareToMoveCol = tempstr[2] - MAGIC_CONVERSION + 1;
		uint8_t squareToMoveRow = tempstr[3] - 48;
		
		reset_LED_board_with_struct();
		//vTaskSuspend(vGameLogicThreadTask);
		
		char bufferPrint[64];
		snprintf(bufferPrint, 64, "square to move: %d, %d\r\n", squareToMoveRow, squareToMoveCol);
		SerialConsoleWriteString(bufferPrint);
		board_ptr[pieceToMoveRow - 1][pieceToMoveCol - 1]->LEDStatus = 'b';
		board_ptr[squareToMoveRow - 1][squareToMoveCol - 1]->LEDStatus = 'p';
		
		
		set_LED_board_with_struct();
		//reset_LED_board_with_struct();
		
		//vTaskResume(vGameLogicThreadTask);
		
		SerialConsoleWriteString("Set best move LED \r\n");
		
		//strncpy(globalBestMoveString, &msgData->message->payload, 9); // TODO
...
//                 mqtt_subscribe(module_inst, IMU_TOPIC, 2, SubscribeHandlerImuTopic);
                mqtt_subscribe(module_inst, CHESS_BEST_MOVE_TOPIC, 2, SubscribeHandlerBestMoveTopic); // TODO
...
static void MQTT_HandleTransactions(void)
{
    ...
    MQTT_HandleFENStringMessages(); 
	MQTT_HandleChessInputMessages(); //TODO
...
{
    char FENString[90];
    if (pdPASS == xQueueReceive(xQueueFENStringBuffer, &FENString, 0)) {
        snprintf(mqtt_msg, 100, FENString);
        mqtt_publish(&mqtt_inst, FENString_TOPIC, mqtt_msg, strlen(mqtt_msg), 1, 0); // TODO
    }
}

static void MQTT_HandleChessInputMessages(void)
{
	char ChessInputString[8];
	if (pdPASS == xQueueReceive(xQueueChessInputBuffer, &ChessInputString, 0)) {
		snprintf(mqtt_msg, 100, ChessInputString);
		mqtt_publish(&mqtt_inst, CHESS_INPUT_TOPIC, mqtt_msg, strlen(mqtt_msg), 1, 0); // TODO
	}
}
...
void vWifiTask(void *pvParameters)
{
    ...
    xQueueFENStringBuffer = xQueueCreate(5, 90*sizeof(char));  // TODO
    xQueueChessInputBuffer = xQueueCreate(5, 8*sizeof(char));  

    if (xQueueWifiState == NULL || xQueueImuBuffer == NULL || xQueueGameBuffer == NULL || xQueueDistanceBuffer == NULL || xQueueFENStringBuffer == NULL || xQueueChessInputBuffer == NULL) {
...
/**
 * Gets the current state of the wifiStateMachine
 */
...
{
	int error = xQueueSend(xQueueChessInputBuffer, ChessInputString, (TickType_t)10);
	return error;
}

int WifiAddFENStringToQueue(char *FENString[90])
{
    int error = xQueueSend(xQueueFENStringBuffer, FENString, (TickType_t)10);
    return error;
} //TODO
/**
 void WifiAddImuDataToQueue(struct ImuDataPacket* imuPacket)
