...
static TaskHandle_t glTaskHandle = NULL; //!< Game logic task handle
...
void vApplicationDaemonTaskStartupHook(void)
{
    ...
    if (I2cInitializeDriver() != STATUS_OK) {
		...
		i2c_begin(); //Initialize OLED
        SerialConsoleWriteString("Initialized I2C Driver!\r\n");
    }
	
	if (SPIInitializeDriver() != STATUS_OK) {
		SerialConsoleWriteString("Error initializing SPI Driver!\r\n");
	} else {
		SerialConsoleWriteString("Initialized SPI Driver!\r\n");
	}
...
/*Initialize Tasks here*/
		if (xTaskCreate(vGameLogicThreadTask, "Game Logic Task", GL_TASK_SIZE, NULL, GL_TASK_PRIORITY, &glTaskHandle) != pdPASS) {
			SerialConsoleWriteString("ERR: GL task could not be initialized!\r\n");
		}

		snprintf(bufferPrint, 64, "Heap after starting GL Task: %d\r\n", xPortGetFreeHeapSize());
		SerialConsoleWriteString(bufferPrint);
...
void configure_port_pins(){
	...
	port_pin_set_output_level(EXT1_PIN_3, 1);//Set PA02 to HIGH initially
}
