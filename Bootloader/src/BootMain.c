...
#define ROW_SIZE 256 //How many bytes in a row
#define MAX_ROWS 123 //Total number of rows to overwrite
...
char flagA_name[] = "0:FlagA.txt";	//Name of test A flag
char flagB_name[] = "0:FlagB.txt";	//Name of test B flag
char testA_bin_file[] = "0:TestA.bin"; //Test A file
char testB_bin_file[] = "0:TestB.bin"; //Test B file
...
FILINFO fno;	//File info object (not used)

uint8_t readBuffer[256]; //Buffer the size of one row
uint32_t numBytesRead = 0; //Keeps track of how many bytes are read from each read operation


/******************************************************************************
* Global Functions
******************************************************************************/
void eraseApplicationMemory(int rowNum) //rowNum indicates which row to erase (rowNum = 0 is the first row of application memory)
{
	char helpStr[100];
	enum status_code nvmError = nvm_erase_row(APP_START_ADDRESS + rowNum * ROW_SIZE);
	if(nvmError != STATUS_OK)
	{
		SerialConsoleWriteString("Erase error");
	}
	
	for(int iter = rowNum * ROW_SIZE; iter < rowNum * (ROW_SIZE + 1); iter++)
	{
...
int main(void)
{
	...
	//If bootloader sees FlagA.txt flag --> update application region with Test A binary
	
	
	flagA_name[0] = LUN_ID_SD_MMC_0_MEM + '0';
	flagB_name[0] = LUN_ID_SD_MMC_0_MEM + '0';
	
	SerialConsoleWriteString("A and B flag test\r\n");
	
	if(0 == f_stat((char const *)flagA_name, &fno)){	//FlagA.txt exists --> continue to update application region with Test A binary
		SerialConsoleWriteString("FlagA.txt found\r\n");	
		int currentRow = 0;
		int numberBytesTotal = 0;
		res = f_open(&file_object, (char const *)testA_bin_file, FA_READ); //Open file
		...
		SerialConsoleWriteString("Starting read of TestA.bin!\r\n");
		
		while(currentRow < MAX_ROWS){
			
			memset(readBuffer, 0, 256);
			eraseApplicationMemory(currentRow); //Erase row
			//Read TestA.bin
			testA_bin_file[0] = LUN_ID_SD_MMC_0_MEM + '0';
			
			int numBytesLeft = ROW_SIZE;
			numBytesRead = 0;
			while(numBytesLeft != 0)
			{
				res = f_read(&file_object, &readBuffer[0], numBytesLeft, &numBytesRead); 
				numBytesLeft -= numBytesRead;
				numberBytesTotal += numBytesRead;
				
				if(numBytesRead != ROW_SIZE){ //This means we have reached the end of the bin file
					SerialConsoleWriteString("Finished reading and writing TestA.bin");
					break;
				}
			...
			/*
			char tempStr[100];
			sprintf(tempStr, "Read pointer: %d | Number of bytes left: %d, | Number of bytes read: %d\r\n", f_tell(&file_object), numBytesLeft, numBytesRead);
			SerialConsoleWriteString(tempStr);
			*/

			int addressOffset = APP_START_ADDRESS + currentRow * ROW_SIZE;
			//Write data to first row. Writes are per page, so we need four writes to write a complete row
			res = nvm_write_buffer (addressOffset, &readBuffer[0], 64);
			res = nvm_write_buffer (addressOffset + 64, &readBuffer[64], 64);
			res = nvm_write_buffer (addressOffset + 128, &readBuffer[128], 64);
			res = nvm_write_buffer (addressOffset + 192, &readBuffer[192], 64);
		...
		SerialConsoleWriteString("ALL WRITE SUCCEEDED!\r\n");
		
		//Delete FlagA.txt
		f_unlink((char const *)flagA_name);
		
		
	} else {											//File not found --> check if TestB.txt exists
		if(0 == f_stat((char const *)flagB_name, &fno)){	//TestB.txt exists --> continue to update application region with Test B binary
			SerialConsoleWriteString("FlagB.txt found\r\n");
			
			int currentRow = 0;
			int numberBytesTotal = 0;
			res = f_open(&file_object, (char const *)testB_bin_file, FA_READ); //Open file
			...
			SerialConsoleWriteString("Starting read of TestB.bin!\r\n");
			...
			while(currentRow < MAX_ROWS){
				...
				while(numBytesLeft != 0)
				{
					...
					if(numBytesRead != ROW_SIZE){ //This means we have reached the end of the bin file
						SerialConsoleWriteString("Finished reading and writing TestB.bin");
						break;
					}
			...
			//Delete FlagB.txt
			f_unlink((char const *)flagB_name);
			
		}
		else 
		{										//No flag exists, check if existing FW is correct, deinitialize HW and jump to application code
			SerialConsoleWriteString("#### NO FLAG EXISTS ####\r\n");
		}
	}	
...
static bool StartFilesystemAndTest(void)
{
	...
	if(sdStatus == CTRL_GOOD) //If the SD card is good we continue mounting the system!
	{
		...
		//Check if sd_mmc_test.txt exists
		
		SerialConsoleWriteString("Check file existence (f_stat)...\r\n");
		
		if(0 != f_stat((char const *)test_file_name, &fno)){
			LogMessage(LOG_INFO_LVL ,"[FAIL]\r\n");
			sdCardPass = false;
			goto main_end_of_test;
		}	
